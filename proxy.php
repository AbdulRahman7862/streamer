...
class Proxy
{
    ...
    private function rewriteDocument($html)
    {
        ...
        // Process all elements with a "src", "href", or "action" attribute.
        ...
        foreach ($all as $el) {
            ...
            if ($el->hasAttribute('href')) {
                ...
                $proxyUrl = $this->generateProxyToken($absoluteUrl);
                error_log("Rewriting href: $url -> $proxyUrl");
            ...
            if ($el->hasAttribute('src')) {
                ...
                // Skip rewriting if the URL is already absolute and points to an external resource.
                if (parse_url($url, PHP_URL_SCHEME) !== null) {
                    continue;
                }

                $proxyUrl = $this->generateProxyToken($absoluteUrl);
                error_log("Rewriting src: $url -> $proxyUrl");
            ...
            if ($el->hasAttribute('action')) {
                ...
                $absoluteUrl = $this->resolveUrl($this->targetUrl, $url);
                $proxyUrl = $this->generateProxyToken($absoluteUrl);
                error_log("Rewriting action: $url -> $proxyUrl");
                $el->setAttribute('action', $proxyUrl);
            }
        }
